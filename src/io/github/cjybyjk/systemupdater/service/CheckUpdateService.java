...
public class CheckUpdateService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, final int startId) {
        if (intent.getAction() == null || !intent.getAction().equals("check")) {
            long nowTime = mDate.getTime() / 1000;
            long lastCheckTime = mSharedPreferences.getLong("last_check", nowTime);
            long secondOfaDay = 86400;
            long checkTriggerTime = nowTime;
            ...
            tManager.set(AlarmManager.RTC_WAKEUP, checkTriggerTime, tPendingIntent);
        } else {
            mDownloadListener = new UpdatesDownloadHelper.DownloadListener() {
                ...
                public void onSuccessListener(File destination) {
                    mSharedPreferences.edit().putLong("last_check", mDate.getTime() / 1000);
    ...
    public void checkUpdate() {
            ...
            public void onSuccess(File destination) {
                ...
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        String tStoragePath = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath();
                        String tFileName = DataFormatUtils.getFileNameFromURL(jsonObject.getString("url"));
                        File tFile = new File(tStoragePath + "/" + tFileName);
        ...
        try {
            ...
            if (mDownloadListener != null) {
                downloadBuilder.setProgressListener(new DownloadClient.ProgressListener() {
                    @Override
